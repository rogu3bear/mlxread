...
> **Update (post-review remediation).** Several findings below have shipped mitigations: **TM-001** — Release now enables Hardened Runtime (`project.yml` `configs.Release`), which with a Developer ID release signature strips `get-task-allow`; **TM-002** — a Sparkle 2 auto-updater with **EdDSA-signed appcasts over HTTPS** now provides update integrity (`MLXRead/Updates/UpdateService.swift`, `docs/updates.md`); and the Accessibility trust grant now has **continuous revocation monitoring** that tears down the event tap the moment trust is removed (`AccessibilityPermissionService.swift`). Rows are annotated **[mitigated]** / **[partially mitigated]** accordingly; residual work is noted inline.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| TM-001 **[partially mitigated]** | Local unprivileged process | App running a read; attacker code runs as same user | Attach to task port and scrape in-memory selection/tensors | Exfiltration of high-sensitivity text | Selected text in memory | **Release Hardened Runtime now enabled** (`project.yml` `configs.Release`); App Sandbox infeasible (global tap + system-wide AX) | `get-task-allow` is only fully stripped once release signing uses **Developer ID** (distribution profile); a locally-signed Release still carries it | Do the public release with Developer ID + notarization (removes `get-task-allow`); optionally minimize selection-buffer lifetime | EndpointSecurity task-port/debugger-attach alerts on the app | Medium | High | **high (until Developer ID release)** |
